...
**NOTE:** the `dotnet` services in this docker-compose solution all read from the same project and folder on the filesystem by mounting the same folder to their /app mountpoint, but using project files in different sub-folders, this is due to the dependency of each on the domain models in the DataClasses project. This can cause sudden restarts or crashes of the service within the container. A simple restart of the affected service (or waiting for the restart if it's caught by the compose restart in the override file and does so automatically) will solve the problem. This is a known problem and will be worked on in time.
