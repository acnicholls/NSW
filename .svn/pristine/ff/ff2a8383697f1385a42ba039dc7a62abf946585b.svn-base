...
CTree.prototype.GetLastNodeByURL = function(stURL)
{
	var st1, st2;

	var iIndex = stURL.indexOf('#');
	if (iIndex == -1)
		st1 = stURL;
	else
		st1 = stURL.substr(0, iIndex);

	for (var i=this.NodeArr.length-1; i>=0; i--)
	{
		iIndex = this.NodeArr[i].stURL.indexOf('#');
		if (iIndex == -1)
			st2 = this.NodeArr[i].stURL;
		else
			st2 = this.NodeArr[i].stURL.substr(0, iIndex);
